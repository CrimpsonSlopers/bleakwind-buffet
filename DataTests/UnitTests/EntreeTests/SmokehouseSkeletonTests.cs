...
namespace BleakwindBuffet.DataTests.UnitTests.EntreeTests
{
    public class SmokehouseSkeletonTests
    {
                                                            ...
                                                            bool includeHashbrowns, bool includePancake)
        {
            ...
            if (!includeSausage)
            {
                Assert.Contains("Hold sausage", entree.SpecialInstructions);
            }
            if (!includeEgg)
            {
                Assert.Contains("Hold eggs", entree.SpecialInstructions);
            }
            if (!includeHashbrowns)
            {
                Assert.Contains("Hold hash browns", entree.SpecialInstructions);
            }
            if (!includePancake)
            {
                Assert.Contains("Hold pancake", entree.SpecialInstructions);
            }
        }
